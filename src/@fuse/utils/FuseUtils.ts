...
class FuseUtils {
	 ...
	 */
	static hasPermission(authArr: string[] | string | undefined, userRole: string): boolean {
		/**
    ...
    */
	...
	static hasPagePermission(pathname: string, user: User) {
		const ignoredPaths = [
			'/',
			'/dashboard',
			'/privacy-policy',
		...
		if (!pathname || ignoredPaths.includes(pathname) || pathname.split('/').filter((p) => !!p).length === 0) {
			return true;
		}
		...
		const comparedFeature =
			pathArr[0] === 'employees' ? 'permissions' : pathArr[0] === 'stage-designs' ? 'stages' : pathArr[0];

		return (
			scopes.findIndex(
				(scope) =>
					scope.feature === comparedFeature &&
					((pathArr[pathArr.length - 1] !== 'new' && pathArr[pathArr.length - 1] !== 'invite') || scope.write)
			) > -1
		);
	}

	static hasOperationPermission(
		feature: string,
		operation: 'view' | 'create' | 'update' | 'delete' | 'exist',
		user: User
	) {
		if (!user) {
			return true;
		...
		return (
			scopes.findIndex(
				(scope) =>
					...
					(operation === 'exist' ||
						(operation === 'delete' || operation === 'create' || operation === 'update'
							? scope.write
							: scope.read))
			) > -1
		);
