...
class FuseAuthorization extends Component<FuseAuthorizationProps, State> {
  ...
  componentDidMount() {
    const { accessGranted, shouldRegisterProfile, pageAccessDenied } =
      this.state;

    if (!accessGranted || shouldRegisterProfile || pageAccessDenied) {
      this.redirectRoute();
    }
  }
  ...
  shouldComponentUpdate(nextProps: FuseAuthorizationProps, nextState: State) {
    ...
    return (
      nextState.accessGranted !== accessGranted ||
      nextState.shouldRegisterProfile !== shouldRegisterProfile ||
      nextState.pageAccessDenied !== pageAccessDenied
    );
  }
  ...
  static getDerivedStateFromProps(props: FuseAuthorizationProps, state: State) {
    ...
    const pageAccessDenied = !FuseUtils.hasPagePermission(pathname, user);
  ...
  redirectRoute() {
    ...
    const { shouldRegisterProfile, pageAccessDenied } = this.state;
