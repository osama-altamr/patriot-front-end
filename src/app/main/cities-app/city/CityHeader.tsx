...
import { useCreateCityMutation, useRemoveCityMutation, useUpdateCityMutation } from '../CitiesApi';
...
function CityHeader() {
	...
	function handleSaveCity() {
		const onSubmit = (data: ICity) => {
			setLoading(true);
			(id && id !== 'new' ? updateCity : createCity)(optimizeCity(getValues()))
				.unwrap()
				.then(() => {
					setLoading(false);
					dispatch(
	...
	return (
		<div className="flex flex-col sm:flex-row flex-1 w-full items-center justify-between space-y-8 sm:space-y-0 py-24 sm:py-32 px-24 md:px-32">
			...
			{id &&
				id !== 'new' &&
				FuseUtils.hasOperationPermission(employeeScopes.cities, 'update', user) && (
					<motion.div
			...
			{(!id || id === 'new' || FuseUtils.hasOperationPermission(employeeScopes.cities, 'update', user)) && (
				<motion.div
					className="flex flex-1 w-full"
					initial={{ opacity: 0, x: 20 }}
					animate={{ opacity: 1, x: 0, transition: { delay: 0.3 } }}
