...
const NotificationApi = api
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      ...
      >({
        query: (filters) => {
          var url = `v1/notifications?page=${filters.page}&pageSize=${filters.pageSize}`;
      ...
      getUnreadNotifications: build.query<
        GetUnreadNotificationsApiResponse,
        GetUnreadNotificationsApiArg
      >({
        query: () => ({
      ...
      dismissAllNotifications: build.mutation<
        DismissAllNotificationsApiResponse,
        DismissAllNotificationsApiArg
      >({
        async queryFn(_arg, _queryApi, _extraOptions, fetchWithBQ) {
          try {
            for (var i = 0; i < _arg.length; i++) {
              await fetchWithBQ({
                url: `v1/notifications/${_arg[i]}`,
                method: "PATCH",
                data: { isSeen: true },
              });
            }
