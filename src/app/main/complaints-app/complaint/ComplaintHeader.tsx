...
function ComplaintHeader() {
	...
	function optimizeComplaint(data: IComplaint) {
		...
		delete complaintData.createdAt;
		delete complaintData.updatedAt;
	...
	function handleSaveComplaint() {
		const onSubmit = (data: IComplaint) => {
			setLoading(true);
			(id && id !== 'new' ? updateComplaint : createComplaint)(optimizeComplaint(getValues()))
				.unwrap()
				.then(() => {
					setLoading(false);
					dispatch(
						showMessage({
							message: t(`COMPLAINT_SAVED_SUCCESSFULLY`),
							variant: 'success',
				...
				.catch((e) => {
					...
					dispatch(
						showMessage({
							message: t(`SOMETHING_WENT_WRONG_WHEN_SAVE_COMPLAINT`),
							variant: 'error',
							autoHideDuration: 2000,
							anchorOrigin: {
								vertical: 'top',
	...
	function handleRemoveComplaint() {
		dispatch(
			openDialog({
				children: (
					<AlertDialog
			...
			<div className="flex flex-col items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
				...
				>
					...
					>
						...
						<span className="flex mx-4 font-medium">{t(`COMPLAINTS`)}</span>
					</Typography>
				</motion.div>
