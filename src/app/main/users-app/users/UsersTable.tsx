...
import { toUserRoleColor, toUserRoleTitle } from '../Utils';
import { useGetUsersQuery, useRemoveUserMutation, useUpdateUserMutation } from '../UsersApi';
import {
	newUsersInstance,
	resetUsers,
...
function UsersTable() {
	...
	const dateFromFilter: string | null = useSelector(selectUsersDateFromFilter);
	...
	const tableActions: ActionProps<IUser>[] = [];

	if (FuseUtils.hasOperationPermission(employeeScopes.users, 'update', user)) {
		tableActions.push({
			title: t('REMOVE'),
			color: 'error',
	...
	function openRemoveUserDialog(userToRemove: IUser) {
		dispatch(
			openDialog({
				children: (
					<AlertDialog
						...
						onSubmit={() => {
							setLoadingRemoveItem(userToRemove.id);
