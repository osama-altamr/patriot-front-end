...
import { useCreateUserMutation, useRemoveUserMutation, useUpdateUserMutation } from '../UsersApi';
...
function UserHeader() {
	...
	function handleSaveUser() {
		const onSubmit = (data: IUser) => {
			setLoading(true);
			(id && id !== 'new' ? updateUser : createUser)(optimizeUser(getValues()))
				.unwrap()
				.then(() => {
					setLoading(false);
					dispatch(
	...
	return (
		<div className="flex flex-col sm:flex-row flex-1 w-full items-center justify-between space-y-8 sm:space-y-0 py-24 sm:py-32 px-24 md:px-32">
			...
			{id && id !== 'new' && FuseUtils.hasOperationPermission(employeeScopes.users, 'update', user) && (
				<motion.div
