...
function StageDesignHeader() {
	...
	function optimizeStageDesign(data: IStageDesign) {
		const stageDesignData = { ...data };
		delete stageDesignData.createdAt;
		delete stageDesignData.updatedAt;
		return stageDesignData;
	}

	function handleSaveStageDesign() {
		const onSubmit = (data: IStageDesign) => {
			setLoading(true);
			(id && id !== 'new' ? updateStageDesign : createStageDesign)(optimizeStageDesign(getValues()))
				.unwrap()
				.then(() => {
					setLoading(false);
					dispatch(
				...
				.catch((e) => {
					...
					dispatch(
						showMessage({
							message: t(`SOMETHING_WENT_WRONG_WHEN_SAVE_STAGE_DESIGN`),
							variant: 'error',
							autoHideDuration: 2000,
							anchorOrigin: {
								vertical: 'top',
		...
		<div className="flex flex-col sm:flex-row flex-1 w-full items-center justify-between space-y-8 sm:space-y-0 py-24 sm:py-32 px-24 md:px-32">
			<div className="flex flex-col items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
				...
				<div className="flex items-center max-w-full">
					...
					>
						...
						>
							{name ? `${localeString(name) ?? ''}` : t(`STAGE_DESIGN`)}
						</Typography>
					</motion.div>
				</div>
			</div>
			...
			{id && id !== 'new' && (
				...
				>
					...
					>
						<span>{t(`REMOVE_STAGE_DESIGN`)}</span>
					</LoadingButton>
				</motion.div>
			)}
			<motion.div
