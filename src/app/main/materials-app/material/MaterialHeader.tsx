...
import { useCreateMaterialMutation, useRemoveMaterialMutation, useUpdateMaterialMutation } from '../MaterialsApi';
...
function MaterialHeader() {
	...
	function optimizeMaterial(data: IMaterial) {
	...
	function handleSaveMaterial() {
		const onSubmit = (data: IMaterial) => {
			setLoading(true);
			(id && id !== 'new' ? updateMaterial : createMaterial)(optimizeMaterial(getValues()))
				.unwrap()
				.then(() => {
					setLoading(false);
					dispatch(
						showMessage({
							message: t(`MATERIAL_SAVED_SUCCESSFULLY`),
							variant: 'success',
				...
				.catch((e) => {
					...
					dispatch(
						showMessage({
							message: t(`SOMETHING_WENT_WRONG_WHEN_SAVE_MATERIAL`),
							variant: 'error',
							autoHideDuration: 2000,
							anchorOrigin: {
								vertical: 'top',
		...
		<div className="flex flex-col sm:flex-row flex-1 w-full items-center justify-between space-y-8 sm:space-y-0 py-24 sm:py-32 px-24 md:px-32">
			<div className="flex flex-col items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
				...
				>
					...
					>
						...
						<span className="flex mx-4 font-medium">{t(`MATERIALS`)}</span>
					</Typography>
				</motion.div>
				...
				<div className="flex items-center max-w-full">
					...
					>
						...
						>
							{name ? `${localeString(name) ?? ''}` : t(`MATERIAL`)}
						</Typography>
					</motion.div>
				</div>
			</div>

			{id && id !== 'new' && FuseUtils.hasOperationPermission(employeeScopes.materials, 'update', user) && (
				<motion.div
			...
			{(!id || id === 'new' || FuseUtils.hasOperationPermission(employeeScopes.materials, 'update', user)) && (
				...
				>
					...
					>
						<span>{t(`${id && id !== 'new' ? 'SAVE' : 'CREATE'}_MATERIAL`)}</span>
					</LoadingButton>
				</motion.div>
			)}
		</div>
