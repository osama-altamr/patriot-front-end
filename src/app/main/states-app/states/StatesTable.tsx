...
import { useGetStatesQuery, useRemoveStateMutation, useUpdateStateMutation } from '../StatesApi';
import {
	newStatesInstance,
	resetStates,
...
function StatesTable() {
	...
	const dateFromFilter: string | null = useSelector(selectStatesDateFromFilter);
	...
	const tableActions: ActionProps<IState>[] = [];

	// Conditionally add actions based on user permissions
	if (FuseUtils.hasOperationPermission(employeeScopes.states, 'update', user)) {
		tableActions.push({
			title: t('REMOVE'),
			color: 'error',
