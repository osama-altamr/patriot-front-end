...
import { useCreateStateMutation, useRemoveStateMutation, useUpdateStateMutation } from '../StatesApi';
...
function StateHeader() {
	...
	const { id, name, isActive } = watch();

	function optimizeState(data: IState) {
	...
	function handleSaveState() {
		const onSubmit = (data: IState) => {
			setLoading(true);
			(id && id !== 'new' ? updateState : createState)(optimizeState(getValues()))
				.unwrap()
				.then(() => {
					setLoading(false);
					dispatch(
						showMessage({
							message: t(`STATE_SAVED_SUCCESSFULLY`),
							variant: 'success',
				...
				.catch((e) => {
					...
					dispatch(
						showMessage({
							message: t(`SOMETHING_WENT_WRONG_WHEN_SAVE_STATE`),
							variant: 'error',
							autoHideDuration: 2000,
							anchorOrigin: {
								vertical: 'top',
	...
	function handleActivateState() {
		setLoadingActivate(true);
		updateState({ id, isActive: true })
			.unwrap()
			.then((action) => {
	...
	function handleDeactivateState() {
		setLoadingDeactivate(true);
		updateState({ id, isActive: false })
			.unwrap()
			.then((action) => {
		...
		<div className="flex flex-col sm:flex-row flex-1 w-full items-center justify-between space-y-8 sm:space-y-0 py-24 sm:py-32 px-24 md:px-32">
			<div className="flex flex-col items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
				...
				>
					...
					>
						...
						<span className="flex mx-4 font-medium">{t(`STATES`)}</span>
					</Typography>
				</motion.div>
				...
				<div className="flex items-center max-w-full">
					...
					>
						...
						>
							{name ? `${localeString(name) ?? ''}` : t(`STATE`)}
						</Typography>
					</motion.div>
				</div>
			</div>
			{id && id !== 'new' && FuseUtils.hasOperationPermission(employeeScopes.states, 'update', user) && (
				<motion.div
				...
				>
					<LoadingButton
						...
						onClick={isActive ? handleDeactivateState : handleActivateState}
						startIcon={
							<Icon
								type="fa6"
								name={isActive ? 'FaRegCircleXmark' : 'FaRegCircleCheck'}
								size="1em"
							/>
						}
						loadingPosition="start"
						loading={isActive ? loadingDeactivate : loadingActivate}
					>
			...
			{(!id || id === 'new' || FuseUtils.hasOperationPermission(employeeScopes.states, 'update', user)) && (
				...
				>
					...
					>
						<span>{t(`${id && id !== 'new' ? 'SAVE' : 'CREATE'}_STATE`)}</span>
					</LoadingButton>
				</motion.div>
			)}
		</div>
