...
import { requiredStringValidation } from '../../utils/validations';
...
function ResetPasswordPage() {
	...
	const { verifyResetCode, signInWithEmailAndPassword } = useJwtAuth();
	const schema = z
		.object({
	...
	const { control, formState, handleSubmit } = useForm<ResetPasswordFormType>({
		mode: 'onChange',
		defaultValues: {
			password: '',
			passwordConfirm: ''
	...
	function onSubmit({ password }: ResetPasswordFormType) {
		setLoading(true);
		verifyResetCode(email, password, resetCode)
			.then(() => {
				signInWithEmailAndPassword(email, password)
					...
					.catch((_errors) => {
						...
						dispatch(
							showMessage({
								message: t('SOMETHING_WENT_WRONG_WHEN_SIGN_IN'),
			...
			.catch((_errors) => {
				...
				dispatch(
					showMessage({
						message: t('SOMETHING_WENT_WRONG_WHEN_CONFIRM_RESET_CODE'),
						autoHideDuration: 2000,
						anchorOrigin: {
							vertical: 'top',
							horizontal: 'right'
		...
		<div className="flex flex-col sm:flex-row items-center md:items-start sm:justify-center md:justify-start flex-1 min-w-0">
			...
			<Paper className="h-full sm:h-auto md:flex md:items-center md:justify-end w-full sm:w-auto md:h-full md:w-1/2 py-8 px-16 sm:p-48 md:p-64 sm:rounded-2xl md:rounded-none sm:shadow md:shadow-none ltr:border-r-1 rtl:border-l-1">
				<div className="flex flex-col w-full max-w-320 sm:w-320 mx-auto sm:mx-0">
					...
					<Typography className="mt-16">{t('RESET_PASSWORD_MESSAGE')}</Typography>
					...
					>
						...
						<LoadingButton
							...
							size="large"
							loading={loading}
							loadingIndicator={t('RESETTING_PASSWORD')}
						>
