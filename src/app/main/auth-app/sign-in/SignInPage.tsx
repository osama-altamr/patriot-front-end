...
import { requiredBooleanValidation, requiredEmailValidation, requiredStringValidation } from '../../utils/validations';
...
function SignInPage() {
	...
	const { signInWithEmailAndPassword } = useJwtAuth();
	const schema = z.object({
		email: requiredEmailValidation(),
		password: requiredStringValidation({ minLength: 8 }),
		rememberMe: requiredBooleanValidation()
	});
	const dispatch = useAppDispatch();
	const [loading, setLoading] = useState(false);
	const { control, formState, handleSubmit } = useForm<SignInFormType>({
		mode: 'onChange',
		defaultValues: {
			email: '',
			password: '',
			...
			<Paper className="h-full sm:h-auto md:flex md:items-center md:justify-end w-full sm:w-auto md:h-full md:w-1/2 py-8 px-16 sm:p-48 md:p-64 sm:rounded-2xl md:rounded-none sm:shadow md:shadow-none ltr:border-r-1 rtl:border-l-1">
				<div className="flex flex-col w-full max-w-320 sm:w-320 mx-auto sm:mx-0">
					...
					<div className="flex items-center mt-24">
						<div
							className="flex-auto mt-px border-t"
							style={{ borderColor: 'darkGrey' }}
						/>
						<Typography className="mx-8">{t('OR')}</Typography>
						<div
							className="flex-auto mt-px border-t"
							style={{ borderColor: 'darkGrey' }}
						/>
