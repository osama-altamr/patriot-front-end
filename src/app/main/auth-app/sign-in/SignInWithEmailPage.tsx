...
type SignInWithEmailFormType = {
  email: string;
};

function SignInWithEmailPage() {
  const { t } = useTranslation("signIn");
  const { requestMagicLink } = useJwtAuth();
  ...
  const { control, formState, handleSubmit } = useForm<SignInWithEmailFormType>(
    {
      mode: "onChange",
      defaultValues: {
        email: "",
  ...
  function onSubmit({ email }: SignInWithEmailFormType) {
    setLoading(true);
      ...
      <Paper className="h-full sm:h-auto md:flex md:items-center md:justify-end w-full sm:w-auto md:h-full md:w-1/2 py-8 px-16 sm:p-48 md:p-64 sm:rounded-2xl md:rounded-none sm:shadow md:shadow-none ltr:border-r-1 rtl:border-l-1">
        <div className="flex flex-col w-full max-w-320 sm:w-320 mx-auto sm:mx-0">
          ...
          <div className="flex items-baseline mt-16 font-medium">
            ...
            <Link className="ms-4" to="/sign-in">
              {t("sIGN_IN")}
            </Link>
          </div>
        </div>
