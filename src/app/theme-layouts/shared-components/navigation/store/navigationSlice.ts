import {
  ...
  export const selectNavigation = createSelector(
	[selectNavigationAll, selectUserRole, selectUser, selectCurrentLanguageId],
	(navigationSimple, userRole, user) => {
	  const navigation =
	  ...
	  function setAdditionalData(data: FuseNavItemType[]): FuseNavItemType[] {
		return data?.map((item) => ({
		  hasPermission:
			Boolean(FuseUtils.hasPermission(item?.auth, userRole)) &&
			FuseUtils.hasPagePermission(item.url, user),
		  ...item,
		  ...(item?.translate && item?.title
			? { title: i18next.t(`navigation:${item?.translate}`) }
			: {}),
		  ...(item?.subTranslate && item?.subtitle
			? { subtitle: i18next.t(`navigation:${item?.subTranslate}`) }
			: {}),
		  ...(item?.children
